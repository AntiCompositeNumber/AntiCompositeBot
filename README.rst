...
.. image:: https://img.shields.io/github/pipenv/locked/python-version/AntiCompositeNumber/AntiCompositeBot
...
Dependencies for this project are managed usin Pipenv_. However, the test runner and the Toolforge containers only use pip, so please run :code:`pipenv_to_requirements -f` after making any changes to Pipfile or Pipfile.lock. You can quickly install all runtime and development dependencies using :code:`pipenv install --dev`.
...
Updating dependencies
    Dependabot will automatically create pull requests if a dependency is out of date. Because of how Dependabot interacts with Pipfiles and the requirements.txt files generated by ``pipenv_to_requirements``, these pull requests should not be accepted. Instead, make sure your git working tree is clear and run::

        $ pipenv update && pipenv run pipenv_to_requirements -f && git commit -a -m "Update dependencies" && git push
    ...
    Then SSH to Toolforge and run the following::
        ...
        $ webservice shell
        $ cd AntiCompositeBot
        $ git pull && pip install --upgrade pip && pip install -r requirements.txt

    ``pip`` must always be run from within ``webservice shell``. The Toolforge bastion runs Python 3.5, but the Kubernetes containers used to run the bot use Python 3.7. Virtual environments created in Python 3.5 won't run correctly in Python 3.7.
